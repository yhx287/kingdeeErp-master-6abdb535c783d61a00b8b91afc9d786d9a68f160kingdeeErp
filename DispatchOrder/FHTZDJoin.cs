...
{
   public  class FHTZDJoin : AbstractBillPlugIn
    {
        public override void AfterBindData(EventArgs e)
        {
            ...
            if (this.View.OpenParameter.Status.Equals(OperationStatus.ADDNEW))
            {
                ...
                DynamicObject assistant = this.Model.GetValue("F_asdf_Assistant1", 0) as DynamicObject;
                if (assistant == null) return;
                string assistantstr = assistant["Id"].ToString();//省份
                DynamicObject city = this.Model.GetValue("F_asdf_Assistant2", 0) as DynamicObject;
                if (city == null) return;
                string citystr = city["Id"].ToString();//市
                DynamicObject county = this.Model.GetValue("F_asdf_Assistant3", 0) as DynamicObject;
        ...
        {
            base.DataChanged(e);
            if (e.Field.Key == "F_asdf_Assistant1" || e.Field.Key == "F_asdf_Assistant2" || e.Field.Key == "F_asdf_Assistant3")
            {
                DynamicObject assistant = this.Model.GetValue("F_asdf_Assistant1", 0) as DynamicObject;
                if (assistant == null) return;
                string assistantstr = assistant["Id"].ToString();//省份
