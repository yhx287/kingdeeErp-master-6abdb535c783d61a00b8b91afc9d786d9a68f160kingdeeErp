...
{
    [Description("销售退货单体积重量的计算")]
    public class volumeWeightXSTHDJS: AbstractBillPlugIn
    {
                ...
                if (this.View.Model.GetEntryRowCount("FEntity") > 0)
                {
                    ...
                    for (int i = 0; i < entrycount; i++)
                    {
                        if (this.Model.GetValue("FMaterialID", i) != null)
                        {
                            ...
                            decimal qty = decimal.Parse(this.View.Model.GetValue("FRealQty", i).ToString());//库存数量
