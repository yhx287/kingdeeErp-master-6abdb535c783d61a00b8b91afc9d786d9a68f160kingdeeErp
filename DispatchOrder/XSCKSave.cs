...
{
    [Description("销售出库单保存的时候更新折扣额，总体积总重量等")]
    public class XSCKSave: AbstractBillPlugIn
    {
        public override void AfterSave(Kingdee.BOS.Core.Bill.PlugIn.Args.AfterSaveEventArgs e)
        {
            ...
            if (e.OperationResult.IsSuccess)
            {
                ...
                if (alldiscount > 0)
                {
                    ...
                    string sql = "/*dialect*/ select F_ZZZ_ZKL,F_ZZZ_ZQAMOUNT,F_zzz_ZQHSPrice,FMATERIALID,FENTRYID,FID from T_SAL_OUTSTOCKENTRY where FID=" + FID;
                    ...
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        ...
                        decimal qty = decimal.Parse(this.View.Model.GetValue("FPRICEUNITQTY", i).ToString());
                    ...
                    for (int i = 0; i < ds.Tables[0].Rows.Count; i++)
                    {
                        ...
                        decimal entrytaxrate = decimal.Parse(this.View.Model.GetValue("FEntryTaxRate", i).ToString());
                        ...
                        if (this.Model.GetValue("FMaterialID", i) != null)
                        {
                            ...
                            #region   计费立方数 计费重量
                            decimal qtyST = decimal.Parse(this.View.Model.GetValue("FREALQTY",i).ToString());
                        ...
                        string sqlDetail = "/*dialect*/ update T_SAL_OUTSTOCKENTRY set F_ASDF_QTY1=" + volume
                            + ",F_ASDF_QTY=" + weight + " where FID=" + FID + " and FEntryID=" + ds.Tables[0].Rows[i]["FEntryId"].ToString();

                        string sqlDetailF = "/*dialect*/ update T_SAL_OUTSTOCKENTRY_F set FDISCOUNT=" + decimal.Parse(strdis) + ",FALLAMOUNT=" + decimal.Parse(strallamountrow) + ",FTAXAMOUNT=" + decimal.Parse(strtaxamount)
                            + ",FPRICE=" + decimal.Parse(strprice) + ",FTAXPRICE=" + decimal.Parse(strtaxprice) + ",FAMOUNT=" + decimal.Parse(strfamount) + ",FDISCOUNTRATE=" + decimal.Parse(strZKL) + " where FID=" + FID + " and FEntryID=" + ds.Tables[0].Rows[i]["FEntryId"].ToString();
